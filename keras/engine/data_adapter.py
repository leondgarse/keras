...
class DataHandler:
  ...
  def __init__(self,
    ...
    self._step_increment = self._steps_per_execution.numpy() - 1
    self._insufficient_data = False
  ...
  def _truncate_execution_to_epoch(self):
    ...
    should_truncate = (
        ...
        self._steps_per_execution.numpy() > self._inferred_steps)
    original_value = self._steps_per_execution.numpy()
    try:
      if should_truncate:
  ...
  def steps(self):
    ...
    while (self._inferred_steps is None or
      ...
      if self._insufficient_data:  # Set by `catch_stop_iteration`.
        break
      spe = self._steps_per_execution.numpy()
      ...
      if can_run_full_execution:
        self._step_increment = spe - 1
        yield self._current_step
        self._current_step += spe
      else:
        ...
        self._steps_per_execution.assign(spe)
  ...
  def _validate_data_handler(self):
    ...
    if self._steps_per_execution.numpy() > 1 and self._inferred_steps is None:
      raise ValueError(
